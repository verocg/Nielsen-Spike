...
% animalName - animal name (string) e.g. 'dingo'
% expDate - date of recording (number, format YYMMDD) e.g. 220105
% taskName - behavioral task (string) e.g. 'RFmap'
% runNo - run number (number),e.g. 1 for RFmap1
% probeID - legacy from Nielsen, number set to 1
% name - initials of person running analysis, e.g. 'AD'
...
% example call with no parallel for loop:
% extractSpikes('\Users\Anita\DATA\TDT\20220105\RFmap1','dingo',220105,'RFmap',1,1,'AD',0,0,0,1,0);
% 
% example call with parfor
% parfor i=0:199
% extractSpikes('\Users\Anita\DATA\TDT\20220105\RFmap1', 'dingo',220105, 'RFmap',1,1, 'AD',0,0,0,200,i)
% end
...
if MUflag==0
    load(fullfile([animalName '-' num2str(expDate) '_' taskName num2str(runNo) '_p' num2str(probeID) '_threshold.mat'])); %generates thresholding
else
    load(fullfile([animalName '-' num2str(expDate) '_' taskName num2str(runNo) '_p' num2str(probeID) '_MUthreshold.mat'])); %generates MUthresholding
    thresholding=MUthresholding;
end
load(fullfile([animalName '-' num2str(expDate) '_' taskName num2str(runNo) '_id.mat'])); %generates id

%compute total channel number
nChannels=sum([id.probes.nChannels]);

%get file size
filename=fullfile(flPath,[taskName num2str(runNo) '_amplifier.dat']);
...
if MUflag==0
    outname=fullfile(flPath,[animalName '-' num2str(expDate) '_' taskName num2str(runNo) '_spike.mat']);
else
    outname=fullfile(flPath,[animalName '-' num2str(expDate) '_' taskName num2str(runNo)  '_MUspike.mat']); 
end
...
if JobID==0
    ...
    save(fullfile(flPath,[animalName '-' num2str(expDate) '_' taskName num2str(runNo) '_id.mat']),'id'); 
    
    if copyToZ==1
        zbase='Z:\Private\Anita\data\processedSpikes';
        save(fullfile(zbase,[animalName '-' num2str(expDate) '_' taskName num2str(runNo) '_id.mat']),'id'); 
    end
